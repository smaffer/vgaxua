...
#define PRINT_LOOP_CODE \
  if (c=='\n') { \
    ...
    if (c>=0 && c<glyphscount) { \
      const byte *fntg=fnt+c*(1+fntheight); \
      const byte fntw=pgm_read_byte(fntg); \
      if (0 && dx+fntw>VGAX_WIDTH) { /*disabled word wrap*/ \
        dy0+=fntheight+vspace; \
        dx=dx0; \
      } \
      /*(inline of bitblit(fntg+1, fntw, fntheight, dx, dy, color & 3))\
        to save from stack overflow*/ \
      byte *sptr=(byte*)fntg+1; \
      byte sheight=fntheight; \
      int dy=dy0; \
      while (sheight--) { \
          byte m=pgm_read_byte(sptr++); \
          if (dy<0 || dy>=VGAX_HEIGHT) \
            goto next; \
...
void VGAXUA::printPROGMEM(byte *fnt, byte glyphscount, byte fntheight, 
                          byte hspace, byte vspace, const char *str, 
                          int dx, int dy0, byte color) {
  int dx0=dx;
  char *pstr=(char*)str;
  char c;
...
void VGAXUA::printSRAM(byte *fnt, byte glyphscount, byte fntheight, byte hspace, 
                       byte vspace, const char *str, int dx, int dy0, 
                       byte color) {
  int dx0=dx;
  char *pstr=(char*)str;
  char c;
